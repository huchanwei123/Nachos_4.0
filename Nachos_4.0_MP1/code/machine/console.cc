...
//	Function used to convert integer to char(ASCII), and then use 
//	consoleIO to write out the result.
//-----------------------------------------------------------------------
...
ConsoleOutput::PutInt(int number)
{
    ...
    while(other != 0){
            digit_one = other % 10;
            other = other / 10;
            num[count+1] = '0' + digit_one;
            count++;
    }
    for(i=count; i>=0; i--){
        WriteFile(writeFileNo, &num[i], sizeof(char));
    }
    putBusy = TRUE;
