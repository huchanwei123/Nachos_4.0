...
OpenForReadWrite(char *name, bool crashOnError)
{
	...
	DEBUG(dbChanwei, "OpenForReadWrite in sysdep.cc ok!");
...
Read(int fd, char *buffer, int nBytes)
{
	...
	DEBUG(dbChanwei, "Read in sysdep.cc ok!");
...
WriteFile(int fd, char *buffer, int nBytes)
{
	...
	DEBUG(dbChanwei, "WriteFile in sysdep.cc ok!");
    ASSERT(retVal == nBytes);
}
...
{
    int retVal = close(fd);
	DEBUG(dbChanwei, "Close in sysdep.cc ok!");
    ASSERT(retVal >= 0); 
    return retVal;
}
