...
class Thread {
  ...
  public:
	...
	// Chanwei add
    Thread(char* debugName, int threadID, int prior);
    // initialize a thread
	// end Chanwei add
	...
	void setBurstTime(double length){ 
		burstTime = length; 
	}
	...
	void setStartTime(int when){ 
		startTime = when; 
	}

    int getPriority(){	// get the priority of thread
		return priority; 
	}
     
	void setPriority(int prior){
        if(prior < 150 && prior >= 0){
        	priority = prior;
        }
    }
     
	void Aging(int increase){
        if((priority + increase) >= 150){
            setPriority(149);
        } 
		else{
            setPriority(priority + increase);
		}
	}
	...
	int getPreBurst(){ return (preBurst); }		// get t(i-1)
	void setPreBurst(int length){
		preBurst = length;
	}

	int getExecutionTime(){ return (executionTime); }
	void setExecutionTime(int length){
		executionTime = length;
	}
	
	void setSleep(){ sleep=1; }
	void resetSleep(){ sleep=0; }
	int isSleep(){ return (sleep); }
	

    int getLastBurst(){ return (lastBurst); }

    void addLastBurst(int length){ 
		lastBurst += length; 
		//cout << "last burst : " << lastBurst << endl;
	}
